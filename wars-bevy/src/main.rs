...
use wars::{game::PlayerNumber, model::UNIT_MAX_HEALTH};
...
#[derive(Resource)]
struct Game {
    state: wars::game::Game,
    players: HashMap<PlayerNumber, Player>,
}

impl Game {
    fn in_turn(&self) -> Option<&Player> {
        self.state
            .in_turn_number()
...
#[derive(Event)]
struct GameEvent(wars::game::Event);
